...
$$
p(y \mid \ldots, \beta_k, \ldots, \beta_{k'}, \ldots, \sigma)
=
p(y \mid \ldots, d  \beta_k, \ldots, \frac{d}{c} \, \beta_{k'}, \ldots,
\sigma).
$$
...
\\[4pt]
& = &\textstyle
\mathsf{beta}(\phi \mid 0.5 + \sum_{n=1}^N y_n, \ \ 0.5 + N - \sum_{n=1}^N y_n).
\end{array}
$$
...
\begin{array}{rcl}
\lambda_1^{(m)}
& \sim & p(\lambda_1 \mid \lambda_2^{(m-1)}, \, \sigma^{(m-1)}, \, y)
\\[6pt]
\lambda_2^{(m)}
& \sim & p(\lambda_2 \mid \lambda_1^{(m)}, \, \sigma^{(m-1)}, \, y)
\\[6pt]
\sigma^{(m)}
& \sim & p(\sigma \mid \lambda_1^{(m)}, \, \lambda_2^{(m)}, \, y).
\end{array}
$$
