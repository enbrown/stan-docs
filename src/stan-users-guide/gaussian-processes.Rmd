...
$$ \begin{aligned}
  f & \sim \mathsf{multivariate\ normal}\left(0, K(x \mid \alpha, \rho)\right) \\ y_i &
...
$$ \begin{aligned}
  f(x \mid a, b, p) & = \dfrac{(a/b)^{p/2}}{2K_p(\sqrt{ab})} x^{p - 1}\exp(-(ax + b
  / x)/2) \\
...
\ = \
\frac{p(\tilde{y}, y \mid \tilde{x},x)}
     {p(y \mid x)}
\ \propto \
p(\tilde{y}, y \mid \tilde{x},x).
$$
...
$$
K(x \mid \alpha, \rho)_{i, j} = \eta^2 \exp(-\dfrac{1}{2 \rho^2}
...
$$ \begin{aligned}
  ...
  f(x) & \sim \mathsf{GP}(m(x), K(x \mid \theta, \phi)) \\
  K(x & \mid \theta) \in \mathbb{R}^{M \times M}, \, f(x), \, m(x) \in \mathbb{R}^M
\end{aligned} $$
...
$$ \begin{aligned} K(x, x^\prime \mid \theta, \phi)_{[m, m^\prime]} = k(x, x^\prime \mid
\theta) k(m, m^\prime \mid \phi) \end{aligned} $$
then our finite dimensional generative model for the above is:
$$ \begin{aligned}
  f & \sim \mathsf{matrixnormal}(m(x), K(x \mid \alpha, \rho), C(\phi)) \\
...
$$ \begin{aligned} f_{[n,]} \sim \mathsf{multivariate\ normal}(m(x)_{[n,]}, K(x \mid \alpha,
...
distributed: $$ \begin{aligned} f_{[,m]} \sim \mathsf{multivariate\ normal}(m(x)_{[,m]}, K(x
  \mid \alpha, \rho) C(\phi)_{[m,m]}) \end{aligned} $$
This also means means that $\mathbb{E}\left[f^T f\right]$ is equal to
$\text{trace}(K(x \mid \alpha, \rho)) \times C$, whereas $\mathbb{E}\left[ff^T\right]$
is $\text{trace}(C) \times K(x \mid \alpha, \rho)$. We can derive this using
...
$$ \begin{aligned}
  ...
  f & \sim \mathsf{matrixnormal}(0, K(x \mid 1.0, \rho), C(\phi)) \\
  \eta & \in \mathbb{R}^{N \times M} \\
  L_C(\phi) & = \text{cholesky\_decompose}(C(\phi)) \\
  L_{K(x \mid 1.0, \rho)} & = \text{cholesky\_decompose}(K(x \mid 1.0, \rho))
\end{aligned} $$
